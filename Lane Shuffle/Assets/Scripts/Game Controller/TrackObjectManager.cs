...
[RequireComponent(typeof(GameController), typeof(LaneManager))]
public class TrackObjectManager : MonoBehaviour
{
    ...
    [SerializeField]
    private float generateZ = 50f;
    [SerializeField]
    private float destroyZ = 10f;
    [SerializeField]
    private float startSectionLength = 10f;
    [SerializeField]
    ...
    {
        while (trackObjects.Count > 0)
        {
            GameObject objectToBeDestroyed = trackObjects[0];
            trackObjects.RemoveAt(0);
        ...
        distanceUntilNextScoreIncrease = startSectionLength + sectionLength;
    ...
    private void Update()
    {
        ...
        distanceUntilNextScoreIncrease -= moveDistance;
        ...
        foreach (GameObject obstacle in trackObjects)
        {
            ...
            {
                obstacle.transform.localPosition += Vector3.back * moveDistance;
        ...
        nextTrackSectionPosition -= moveDistance;
        while (nextTrackSectionPosition < generateZ)
        {
            ...
            for (int i = 0; i < upcomingSections.Length; i++)
            {
                ...
                else
                {
                    ...
                    upcomingSections[i][0].Build(lanes[i], nextTrackSectionPosition, sectionLength, this);
